...
import { Eye, PackageSearch, Printer, Filter, FileSpreadsheet } from 'lucide-react';
...
const fetchOrders = async (dateRange?: DateRange, searchTerm: string = ''): Promise<Order[]> => {
  ...
  if (searchTerm) filters.orderNumber = searchTerm; // Firestore query will handle this if possible, or client-side filter

  const orders = await dataStore.getOrders(filters);
  // dataStore.getOrders now returns orders with Date objects for createdAt
  return orders;
};
...
export default function OrdersPage() {
  ...
  const { data: orders = [], isLoading } = useQuery<Order[]>({
    queryKey: ['orders', dateRange, searchTerm],
    queryFn: () => fetchOrders(dateRange, searchTerm),
  });
  ...
  const handlePrintOrder = () => {
    if (printOrderRef.current && selectedOrder) {
        ...
        if (printWindow) {
            ...
            // Inject styles directly for better compatibility
            const styles = `
                body { 
        ...
        toast({ title: "Print Error", description: "No order content to print or order not selected.", variant: "destructive" });
    }
  };
  ...
  const handleExportOrders = () => {
    ...
    const dataToExport = orders.map(order => ({
      ...
      'Firestore Order ID': order.id, // Firestore ID
    ...
    toast({ title: 'Export Successful', description: 'Orders data has been exported to Excel.' });
  };
  ...
  return (
    <div className="space-y-6 p-4 md:p-6">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
        <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-primary">Order History</h1>
        ...
        <Card className="shadow-lg">
          <CardContent className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-muted-foreground mt-1">
                {searchTerm || dateRange ? "No orders match your current filters." : "There are no orders in the system yet."}
            </p>
          </CardContent>
        </Card>
      ) : (
      <Card className="shadow-lg">
        <CardHeader>
            ...
            <CardDescription>
              A list of all processed orders.
              {dateRange?.from && (
            ...
            <>
                <div ref={printOrderRef} className="printable-bill-content space-y-3 text-xs"> 
                    <div className="header text-center mb-3">
                        <h2 className="shop-name text-base font-bold">{SHOP_NAME}</h2>
                    ...
                    <div className="total-section text-right mt-2">
                     ...
                     <div className="barcode-section text-center mt-3 barcode-svg"> {/* Added barcode-svg class */}
