...
import { Search, ScanLine, PlusCircle, Trash2, Printer, CheckCircle, ShoppingCart, Loader2 } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import { SHOP_NAME, CONTACT_NUMBERS } from '@/lib/constants';
...
export default function BillingPage() {
  ...
  const generateOrderNumber = async () => {
    const lastOrders = await dataStore.getOrders({ orderBy: 'orderNumber', orderDirection: 'desc', limit: 1 });
    let lastOrderNumberSuffix = 0;
    if (lastOrders.length > 0 && lastOrders[0].orderNumber) {
      const lastOrderNumStr = lastOrders[0].orderNumber.split('-')[1];
      if (lastOrderNumStr) {
        lastOrderNumberSuffix = parseInt(lastOrderNumStr, 10);
      }
    }
    return `ORD-${String(isNaN(lastOrderNumberSuffix) ? 1 : lastOrderNumberSuffix + 1).padStart(4, '0')}`;
  };
  ...
  const finalizeOrderMutation = useMutation({
    ...
    onError: (error) => {
      if (error instanceof Error && (error.message.includes('Insufficient stock') || error.message.includes('not found during finalization') || error.message.includes('Empty bill'))) {
        // Specific toasts already handled
      } else {
        toast({ title: 'Order Failed', description: `Error finalizing order: ${error instanceof Error ? error.message : 'Unknown error'}`, variant: 'destructive' });
      }
    },
  });
    ...
    <div ref={ref} className="printable-bill-actual-content p-0 m-0 font-sans text-sm bg-white text-black">
      <style type="text/css">
        {`
          @media print {
            ...
            .print-bill-wrapper-active .header { text-align: center; margin-bottom: 20px; }
            .print-bill-wrapper-active .shop-name { font-size: 1.8em; font-weight: bold; color: #1a1a1a !important; margin-bottom: 5px; }
            .print-bill-wrapper-active .contact-info { font-size: 0.9em; color: #404040 !important; margin-bottom: 20px; }
            .print-bill-wrapper-active .order-details { margin-bottom: 20px; padding-bottom: 10px; border-bottom: 1px dashed #ccc !important; }
            ...
            .print-bill-wrapper-active .items-table { width: 100%; border-collapse: collapse; margin-bottom: 20px; }
            .print-bill-wrapper-active .items-table th, .print-bill-wrapper-active .items-table td { border: 1px solid #ddd !important; padding: 8px; text-align: left; font-size: 0.9em; }
            ...
            .print-bill-wrapper-active .total-section { text-align: right; margin-top: 20px; }
            ...
            .print-bill-wrapper-active .footer { text-align: center; margin-top: 30px; padding-top: 15px; border-top: 1px solid #eee !important; font-size: 0.85em; color: #505050 !important; }
            .print-bill-wrapper-active .barcode-section { text-align: center; margin-top: 20px; }
      ...
      <div className="print-container">
        ...
        <div className="footer">
          ...
          <p>Visit us again at Grameena Home Foods, A.R Organics.</p>
        </div>
      </div>
    </div>
  );
